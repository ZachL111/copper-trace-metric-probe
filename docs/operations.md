...
`copper-trace-metric-probe` is centered on observability work in Scala. The technical target is: Package a Scala local lab for metric analysis with negative fixtures, human-readable error snapshots, and documented operating limits.
...
The score is deterministic. The current threshold is 168. Risk has a penalty of 5, latency has a penalty of 2, and weight contributes a bonus of 2.
